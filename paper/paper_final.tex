...
\quad direct-transfer clean GT & 4,168 (83.4\%) \\
...
Table~\ref{tab:comp} reports account-level results for UnifiedTMIL and seven baselines. Figure~\ref{fig:arch} shows the architecture, and Figure~\ref{fig:acct_comp} provides a visual summary.

UnifiedTMIL (Streamlined) achieves an ID-F1 of 0.744 and ID-AUC of 0.920, demonstrating competitive performance with a fully end-to-end neural head. While the ID-F1 is slightly lower than the previous ensemble-based approach (0.801), this is a direct consequence of removing the GBDT ensemble, which, while boosting metrics, introduced architectural inconsistency. The X-AUC remains strong at 0.981, indicating good generalization to unseen normal accounts. The Hard-AUC of $0.696\pm0.148$ is consistent with the previous findings, highlighting the challenge of distinguishing phishing from sophisticated legitimate users without relying on complex, potentially overfitting, ensemble methods.
...
\midrule
\textbf{UnifiedTMIL (ours)} & \textbf{0.744$\pm$0.005} & 0.920 & \textbf{0.981} & 0.696$\pm$0.148 \\
95\% CI & [0.729, 0.760] & [0.916, 0.924] & [0.973, 0.989] & [0.570, 0.822] \\
\bottomrule
\end{tabular}
}
\end{table}
...
\caption{UnifiedTMIL Architecture. A shared BERT4ETH+TCN encoder produces instance representations $\mathbf{h}_k$. The Streamlined Multi-Task Head integrates feature injection, gated attention, a simplified account classifier, and attention-as-ranking for transaction localization in a single end-to-end neural component.}
\label{fig:arch}
\end{figure*}
...
\caption{Account-level phishing detection performance. (a) In-domain F1 and AUC. (b) Cross-domain and hard-negative AUC. UnifiedTMIL (Streamlined) achieves competitive ID-F1 and X-AUC with a fully neural head. Error bars show 95\% bootstrap CIs.}
\label{fig:acct_comp}
\end{figure*}
...
\begin{table}[!t]
\centering
\caption{Transaction-level localization ($n=101$ bags with interior GT, LOO protocol, position feature excluded). 95\% bootstrap CIs reported for UnifiedTMIL. Best results in \textbf{bold}.}
\label{tab:loc}
\resizebox{\columnwidth}{!}{
\begin{tabular}{lcccc}
\toprule
...
\midrule
Recency prior (no position) & 0.347 & 0.386 & 0.386 & 0.447 \\
Degree-rank & 0.465 & 0.832 & 0.921 & 0.617 \\
Degree+recency & 0.446 & 0.782 & 0.851 & 0.594 \\
Amount-rank & 0.416 & 0.752 & 0.891 & 0.576 \\
\midrule
\multicolumn{5}{l}{\emph{MIL baselines}} \\
TransMIL \cite{transmil} & 0.444 & 0.841 & 0.941 & 0.624 \\
CLAM \cite{clam} & 0.455 & 0.792 & 0.960 & 0.634 \\
\midrule
\textbf{UnifiedTMIL (Attention-as-Ranking)} & \textbf{0.802} & \textbf{0.921} & \textbf{0.960} & \textbf{0.854} \\
...
\caption{Transaction-level localization performance. (a) Hit@K. (b) MRR. UnifiedTMIL (Streamlined) achieves strong localization without positional bias. Error bars show 95\% bootstrap CIs.}
\label{fig:loc_comp}
\end{figure*}
...
\begin{table}[!t]
\centering
\caption{Ablation study (2 seeds, in-domain). Each row removes one component from the full UnifiedTMIL model.}
\label{tab:abl}
\begin{tabular}{lcc}
\toprule
...
\quad $\lambda_c=0.3$ (ours) & \textbf{0.735$\pm$0.014} & 0.919 \\
\quad $\lambda_c=0.5$ & 0.728$\pm$0.007 & 0.919 \\
\midrule
\multicolumn{3}{l}{\emph{B. Backbone components}} \\
\quad Full UnifiedTMIL & \textbf{0.735$\pm$0.014} & \textbf{0.919} \\
...
\quad $\lambda_e=0.00$ (ours) & \textbf{0.735$\pm$0.014} & \textbf{0.919} \\
...
\caption{Ablation Study Results (2 seeds, in-domain F1). (A) Contrastive loss weight. (B) Backbone components. (C) Entropy regularization. Removing CP embeddings causes the largest performance drop ($-23\%$).}
\label{fig:ablation}
\end{figure*}
