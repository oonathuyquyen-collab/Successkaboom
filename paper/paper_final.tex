...
UnifiedTMIL achieves an ID-F1 of $0.744\pm0.005$ and ID-AUC of $0.920$, demonstrating competitive performance with a fully end-to-end neural head. The X-AUC remains strong at $0.981\pm0.009$, indicating good generalization to unseen normal accounts. The Hard-AUC of $0.696\pm0.148$ highlights the challenge of distinguishing phishing from sophisticated legitimate users without relying on complex, potentially overfitting, ensemble methods.
...
\midrule
\textbf{UnifiedTMIL (ours)} & \textbf{0.744$\pm$0.005} & \textbf{0.920} & \textbf{0.981$\pm$0.009} & \textbf{0.696$\pm$0.148} \\
...
\caption{Account-level phishing detection performance. (a) In-domain F1 and AUC. (b) Cross-domain and hard-negative AUC. UnifiedTMIL achieves competitive ID-F1 and X-AUC with a fully neural head. Error bars show 95\% bootstrap CIs.}
\label{fig:acct_comp}
\end{figure*}
...
UnifiedTMIL's attention-as-ranking approach achieves Hit@1 $0.832\pm0.029$ and MRR $0.880\pm0.021$. This represents a strong performance for an end-to-end neural model without explicit positional features or a separate ranking model. The current results demonstrate that a fully neural, architecturally consistent model can achieve highly competitive localization. The improvement over heuristic priors (e.g., Degree-rank Hit@1 0.465) confirms the value of learned attention and injected features.
...
\midrule
\textbf{UnifiedTMIL (Attention-as-Ranking)} & \textbf{0.832} & \textbf{0.931} & \textbf{0.941} & \textbf{0.880} \\
95\% CI & [0.772, 0.881] & [0.891, 0.960] & [0.901, 0.970] & [0.839, 0.911] \\
\bottomrule
\end{tabular}
}
\end{table}
...
\caption{Transaction-level localization performance. (a) Hit@K. (b) MRR. UnifiedTMIL achieves strong localization without positional bias. Error bars show 95\% bootstrap CIs.
\label{fig:loc_comp}
\end{figure*}
...
\quad $\lambda_c=0.1$ (ours) & \textbf{0.737$\pm$0.004} & \textbf{0.921} \\
\quad $\lambda_c=0.5$ & 0.728$\pm$0.007 & 0.919 \\
\midrule
\multicolumn{3}{l}{\emph{B. Backbone components}} \\
\quad Full UnifiedTMIL & \textbf{0.737$\pm$0.004} & \textbf{0.921} \\
...
\quad $\lambda_e=0.00$ (ours) & \textbf{0.737$\pm$0.004} & \textbf{0.921} \\
